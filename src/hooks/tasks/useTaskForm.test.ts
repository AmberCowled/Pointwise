...
vi.mock('@pointwise/lib/datetime', async () => {
  ...
  return {
    ...
    extractTime: vi.fn((date: Date, defaultTime: string, _tz: string) => defaultTime),
...
describe('useTaskForm', () => {
  ...
  describe('error handling', () => {
    ...
    it('should clear date order error', () => {
      ...
      act(() => {
        result.current.setErrors({ dateOrder: 'Start date must be before due date' });
  ...
  describe('field IDs', () => {
    ...
    it('should generate different IDs for different instances', () => {
      const { result: result1 } = renderHook(() => useTaskForm({ mode: 'create' }));
      const { result: result2 } = renderHook(() => useTaskForm({ mode: 'create' }));

      expect(result1.current.titleFieldId).not.toBe(result2.current.titleFieldId);
    });
  });
});
