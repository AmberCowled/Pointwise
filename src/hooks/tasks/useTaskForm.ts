...
import { CORE_TASK_CATEGORIES, isCoreTaskCategory } from "@pointwise/lib/categories";
import { DateTimeDefaults, extractTime, toLocalDateTimeString } from "@pointwise/lib/datetime";
...
export interface UseTaskFormReturn {
	...
	handleChange: <T extends keyof TaskFormValues>(key: T, value: TaskFormValues[T]) => void;
...
export function useTaskForm(options: UseTaskFormOptions): UseTaskFormReturn {
	...
	const sourceData = isEditingSeries && recurringTaskData ? recurringTaskData : editingTask;
	...
	const getInitialDateTime = (
		...
		if (isRecurringData) {
			...
			const dateStr = typeof startDate === "string" ? startDate.split("T")[0] : undefined;
			return {
				date: dateStr,
			...
			const startDate = taskData.startDate;
			const dateStr =
				startDate instanceof Date ? startDate.toISOString().split("T")[0] : startDate || undefined;
			return {
				date: dateStr,
	...
	const [customCategory, setCustomCategory] = useState<string>(() =>
		initialCategorySelection === CUSTOM_CATEGORY_OPTION_VALUE ? initialCategoryValue : "",
	);
	const [selectedCategory, setSelectedCategory] = useState<string>(initialCategorySelection);

	// Form state
	const [form, setForm] = useState<TaskFormValues>(() => ({
		id: editingTask?.id,
		title:
			isEditingSeries && recurringTaskData ? recurringTaskData.title : (editingTask?.title ?? ""),
		category: initialCategoryValue,
		xpValue:
			isEditingSeries && recurringTaskData ? recurringTaskData.xpValue : (editingTask?.xp ?? 50),
		context:
			isEditingSeries && recurringTaskData
		...
		recurrence: isEditingSeries && recurringTaskData ? recurringTaskData.recurrence : "none",
		recurrenceDays: isEditingSeries && recurringTaskData ? recurringTaskData.recurrenceDays : [],
		recurrenceMonthDays:
			isEditingSeries && recurringTaskData ? recurringTaskData.recurrenceMonthDays : [],
		timesOfDay: isEditingSeries && recurringTaskData ? recurringTaskData.timesOfDay : [],
	}));
	...
	const [pendingSubmission, setPendingSubmission] = useState<TaskFormValues | null>(null);
