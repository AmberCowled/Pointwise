...
import type { TaskFormValues, RecurringTaskData } from '@pointwise/app/components/dashboard/tasks/form/types';
...
export function useTaskForm(options: UseTaskFormOptions): UseTaskFormReturn {
  ...
  const sourceData = isEditingSeries && recurringTaskData 
    ? recurringTaskData 
    : editingTask;
  ...
  const getInitialDateTime = (task: DashboardTask | RecurringTaskData | null, isRecurringData: boolean) => {
    ...
    if (isRecurringData) {
      ...
      const startDate = taskData.startDate;
      const dateStr = startDate instanceof Date ? startDate.toISOString().split('T')[0] : startDate || undefined;
      return {
        date: dateStr,
  ...
  const startDateTime = isEditingSeries && recurringTaskData
    ? getInitialDateTime(recurringTaskData, true)
    : getInitialDateTime(editingTask, false);

  const dueDateTime = editingTask ? {
    date: editingTask.dueDate instanceof Date ? editingTask.dueDate.toISOString().split('T')[0] : editingTask.dueDate || undefined,
  ...
  const [form, setForm] = useState<TaskFormValues>(() => ({
    id: editingTask?.id,
    title: isEditingSeries && recurringTaskData 
      ? recurringTaskData.title 
      : editingTask?.title ?? '',
    category: initialCategoryValue,
    xpValue: isEditingSeries && recurringTaskData
      ? recurringTaskData.xpValue
      : editingTask?.xp ?? 50,
    context: isEditingSeries && recurringTaskData
    ...
    recurrence: isEditingSeries && recurringTaskData
      ? recurringTaskData.recurrence
      : 'none',
    recurrenceDays: isEditingSeries && recurringTaskData
      ? recurringTaskData.recurrenceDays
      : [],
    recurrenceMonthDays: isEditingSeries && recurringTaskData
      ? recurringTaskData.recurrenceMonthDays
      : [],
    timesOfDay: isEditingSeries && recurringTaskData
      ? recurringTaskData.timesOfDay
      : [],
  }));
  ...
  const handleChange = useCallback(<T extends keyof TaskFormValues>(
    key: T,
