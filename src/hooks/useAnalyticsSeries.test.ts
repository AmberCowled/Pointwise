...
describe("useAnalyticsSeries", () => {
	...
	it("should build analytics snapshot with tasks", () => {
		...
		const { result } = renderHook(() => useAnalyticsSeries(tasks, "7d", "en-US", "UTC"));
	...
	it("should handle empty tasks array", () => {
		const { result } = renderHook(() => useAnalyticsSeries([], "7d", "en-US", "UTC"));
	...
	it("should handle different ranges", () => {
		...
		const { result: result7d } = renderHook(() => useAnalyticsSeries(tasks, "7d", "en-US", "UTC"));
	...
	it("should handle different locales", () => {
		...
		const { result: resultEn } = renderHook(() => useAnalyticsSeries(tasks, "7d", "en-US", "UTC"));
		const { result: resultFr } = renderHook(() => useAnalyticsSeries(tasks, "7d", "fr-FR", "UTC"));
	...
	it("should handle different timezones", () => {
		...
		const { result: resultUtc } = renderHook(() => useAnalyticsSeries(tasks, "7d", "en-US", "UTC"));
	...
	it("should handle non-array tasks input", () => {
		const { result } = renderHook(() => useAnalyticsSeries(null as any, "7d", "en-US", "UTC"));
	...
	it("should memoize results when inputs unchanged", () => {
		...
		const { result, rerender } = renderHook(
			({ tasks, range, locale, timeZone }) => useAnalyticsSeries(tasks, range, locale, timeZone),
			{
				initialProps: {
					tasks,
					range: "7d" as const,
