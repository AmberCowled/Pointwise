...
describe('useTaskFilters', () => {
  ...
  describe('scheduledTasks', () => {
    it('should filter tasks for day view', () => {
      ...
      const { result } = renderHook(() =>
        useTaskFilters(tasks, selectedDate, locale, timeZone, referenceTimestamp, 'day'),
      );

      expect(result.current.scheduledTasks.length).toBeGreaterThanOrEqual(1);
      expect(result.current.scheduledTasks.some((t) => t.id === '1')).toBe(true);
    });
    ...
    it('should filter tasks for week view', () => {
      const tasks = [
        createMockTask({
          id: '1',
          startDate: '2025-01-15', startTime: '09:00:00', // Day 1
        }),
        createMockTask({
          id: '2',
          startDate: '2025-01-20', startTime: '09:00:00', // Day 6
        }),
        createMockTask({
          id: '3',
          startDate: '2025-01-25', startTime: '09:00:00', // Outside week
        }),
      ];

      const { result } = renderHook(() =>
        useTaskFilters(tasks, selectedDate, locale, timeZone, referenceTimestamp, 'week'),
      );

      expect(result.current.scheduledTasks.length).toBeGreaterThanOrEqual(2);
      expect(result.current.scheduledTasks.some((t) => t.id === '1')).toBe(true);
      expect(result.current.scheduledTasks.some((t) => t.id === '2')).toBe(true);
    });
    ...
    it('should filter tasks for month view', () => {
      const tasks = [
        ...
        createMockTask({
          id: '2',
          startDate: '2025-01-30', startTime: '09:00:00', // Day 15
        }),
        createMockTask({
          id: '3',
          startDate: '2025-02-10', startTime: '09:00:00', // Outside month
        }),
      ];

      const { result } = renderHook(() =>
        useTaskFilters(tasks, selectedDate, locale, timeZone, referenceTimestamp, 'month'),
      );
    ...
    it('should exclude completed tasks', () => {
      const tasks = [
        createMockTask({
          id: '1',
          startDate: '2025-01-15', startTime: '09:00:00',
          completed: false,
        }),
        createMockTask({
          id: '2',
          startDate: '2025-01-15', startTime: '10:00:00',
          completed: true,
        }),
      ];
      ...
      expect(result.current.scheduledTasks.every((t) => !t.completed)).toBe(true);
    });
    ...
    it('should handle tasks with only dueAt', () => {
      const tasks = [
        createMockTask({
          id: '1',
          dueDate: '2025-01-15', dueTime: '17:00:00',
    ...
    it('should handle tasks that overlap date range', () => {
      const tasks = [
        createMockTask({
          id: '1',
          startDate: '2025-01-14', startTime: '09:00:00',
          dueDate: '2025-01-16', dueTime: '17:00:00', // Overlaps selected date
        }),
      ];
  ...
  describe('selectedDateLabel', () => {
    it('should format date label for day view', () => {
      const { result } = renderHook(() =>
        useTaskFilters([], selectedDate, locale, timeZone, referenceTimestamp, 'day'),
      );
    ...
    it('should format date range for week view', () => {
      const { result } = renderHook(() =>
        useTaskFilters([], selectedDate, locale, timeZone, referenceTimestamp, 'week'),
      );
    ...
    it('should format date range for month view', () => {
      const { result } = renderHook(() =>
        useTaskFilters([], selectedDate, locale, timeZone, referenceTimestamp, 'month'),
      );
  ...
  describe('selectedDateInputValue', () => {
    it('should return date key for selected date', () => {
      ...
      expect(result.current.selectedDateInputValue).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('should handle non-array tasks input', () => {
      const { result } = renderHook(() =>
        useTaskFilters(null as any, selectedDate, locale, timeZone, referenceTimestamp, 'day'),
      );
