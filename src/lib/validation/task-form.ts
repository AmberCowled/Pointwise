...
import { isCoreTaskCategory, MAX_CUSTOM_CATEGORY_LENGTH } from "@pointwise/lib/categories";
 ...
 */
export function validateTaskForm(options: ValidateTaskFormOptions): ValidationResult {
	const { form, selectedCategory, customCategory, hasStart = false, hasDue = false } = options;
	...
	if (startDateTime && dueDateTime && startDateTime.getTime() > dueDateTime.getTime()) {
		errors.dateOrder = "Start date/time must be before or equal to due date/time";
	}
...
export function validateField(
	...
	switch (field) {
		...
		case "category": {
			...
			if (!isCoreTaskCategory(trimmed) && trimmed.length > MAX_CUSTOM_CATEGORY_LENGTH) {
