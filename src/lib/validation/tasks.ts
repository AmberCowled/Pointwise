...
export function parseCreateTaskBody(
  ...
  if (startDate && dueDate) {
    const startDateTime = startTime ? new Date(`${startDate}T${startTime}`) : new Date(`${startDate}T00:00:00`);
...
export function parseUpdateTaskBody(
  ...
  if (data.startDate && data.dueDate) {
    const startDateTime = data.startTime ? new Date(`${data.startDate}T${data.startTime}`) : new Date(`${data.startDate}T00:00:00`);
    const dueDateTime = data.dueTime ? new Date(`${data.dueDate}T${data.dueTime}`) : new Date(`${data.dueDate}T23:59:59`);
  ...
  if ('recurrenceMonthDays' in body) {
    data.recurrenceMonthDays = sanitizeIntegerArray(
      body.recurrenceMonthDays,
      1,
      31,
    );
