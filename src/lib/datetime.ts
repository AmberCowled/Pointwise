...
export function startOfDay(input: DateInput, timeZone: string = DEFAULT_TIME_ZONE): Date {
...
export function toDateKey(input: DateInput, timeZone: string = DEFAULT_TIME_ZONE): string {
	const date = toDate(input) ?? new Date();
	const parts = getDateTimeParts(date, timeZone);
...
export function toLocalDateTimeString(
	...
	const targetUtc = Date.UTC(parts.year, parts.month - 1, parts.day, hoursNum, minutesNum);
...
export function mergeDateAndTime(
	...
	let utcDate = new Date(Date.UTC(targetYear, targetMonth - 1, targetDay, hours, minutes, 0));
	...
	if (currentDateKey !== targetDateKey) {
		...
		if (dayDiff !== 0) {
			...
			utcDate = new Date(Date.UTC(targetYear, targetMonth - 1, targetDay, hours, minutes, 0));
			...
			utcDate = new Date(Date.UTC(targetYear, targetMonth - 1, targetDay, utcHour, utcMinute, 0));
