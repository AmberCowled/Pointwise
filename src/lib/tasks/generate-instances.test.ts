...
const createTemplateTask = (overrides: Partial<DashboardTask> = {}): DashboardTask => ({
	id: "template-1",
	title: "Daily Task",
...
describe("generateRecurringInstances", () => {
	...
	describe("edited instances", () => {
		it("should skip edited instances", () => {
			...
			futureDate.setDate(futureDate.getDate() + 1);
			const startDateStr = futureDate.toISOString().split("T")[0];
			const editedDate = new Date(futureDate.getTime() + 2 * 24 * 60 * 60 * 1000); // +2 days
			...
			const editedInstance = instances.find((inst) => inst.recurrenceInstanceKey === editedKey);
		...
		it("should use existing edited instances from database", () => {
			...
			const existingInstance = createInstanceTask(template.id, editedKey, editedDateStr, {
				title: "Edited Task",
			...
			const instances = generateRecurringInstances(template, dateRange, "UTC", [existingInstance]);

			// Should include the edited instance
			const foundInstance = instances.find((inst) => inst.recurrenceInstanceKey === editedKey);
	...
	describe("edge cases", () => {
		...
		it("should filter instances to date range", () => {
			...
			instances.forEach((instance) => {
				const date = new Date(instance.startDate as string);
				expect(date.getTime()).toBeGreaterThanOrEqual(dateRange.start.getTime());
