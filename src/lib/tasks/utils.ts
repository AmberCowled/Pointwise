...
export function isTaskTemplate(
  ...
  return !task.isRecurringInstance && task.recurrencePattern !== undefined && task.recurrencePattern !== null;
}
...
export function isOneTimeTask(
  task: Pick<Task | DashboardTask, 'isRecurringInstance' | 'recurrencePattern' | 'sourceRecurringTaskId'>
): boolean {
