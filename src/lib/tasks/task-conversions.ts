...
import type { UpdateTaskRequest, RecurrencePattern } from '@pointwise/lib/api/types';
...
export async function updateSingleTask(
  ...
  if (isInstance && task.sourceRecurringTaskId) {
    ...
    if (template) {
      const editedKeys = ((template as any).editedInstanceKeys as string[]) || [];
      const instanceKey = task.recurrenceInstanceKey || 
