...
import type { RecurrencePattern, UpdateTaskRequest } from "@pointwise/lib/api/types";
		...
		startDate:
			"startDate" in updates
				? updates.startDate
					...
					: null
				: task.startDate,
		startTime: "startTime" in updates ? (updates.startTime ?? null) : task.startTime,
		dueDate:
			"dueDate" in updates ? (updates.dueDate ? new Date(updates.dueDate) : null) : task.dueDate,
		dueTime: "dueTime" in updates ? (updates.dueTime ?? null) : task.dueTime,
	};
}
 ...
 */
export function buildRecurrencePattern(options: BuildRecurrencePatternOptions): RecurrencePattern {
 ...
 */
export function setRecurrenceTemplateFields(pattern: RecurrencePattern): Record<string, unknown> {
	return {
