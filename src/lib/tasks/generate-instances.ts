...
export function generateRecurringInstances(
  ...
  for (const occurrenceDate of filteredOccurrences) {
    ...
    const times =
      pattern.timesOfDay && pattern.timesOfDay.length > 0
        ? pattern.timesOfDay
        ...
        const templateStartDate =
          typeof template.startDate === "string"
            ...
            : template.startDate
              ? new Date(template.startDate)
              : null;
        const templateDueDate =
          typeof template.dueDate === "string"
            ? new Date(template.dueDate)
            : new Date(template.dueDate);

        if (templateStartDate) {
          const daysDiff = Math.floor(
            (templateDueDate.getTime() - templateStartDate.getTime()) /
              (1000 * 60 * 60 * 24),
          );
