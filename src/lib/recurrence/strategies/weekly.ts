...
import { addDays, mergeDateAndTime, startOfDay, toDateKey } from "@pointwise/lib/datetime";
import { DAYS_IN_WEEK, MS_PER_DAY, RECURRENCE_CONFIG } from "../config";
...
export class WeeklyRecurrenceStrategy implements RecurrenceStrategy {
	generateOccurrences(
		...
		const daySet = new Set(
			config.daysOfWeek && config.daysOfWeek.length > 0 ? config.daysOfWeek : [base.getDay()],
		);
		...
		while (occurrenceCount < maxOccurrences) {
			...
			for (let dayOffset = 0; dayOffset < DAYS_IN_WEEK; dayOffset += 1) {
				const currentDay = addDays(base, weekOffset * DAYS_IN_WEEK + dayOffset, timeZone);
	...
	findNextOccurrence(input: NextOccurrenceInput): Date | null {
		...
		const daySet = new Set(daysOfWeek && daysOfWeek.length > 0 ? daysOfWeek : [base.getDay()]);

		const todayKey = toDateKey(new Date(), timeZone);
		const startOffset = lastTaskDate ? 1 : 0;
		const maxWeeks = RECURRENCE_CONFIG.weekly.maxWeeksToSearch;

		for (let offset = startOffset; offset < maxWeeks * DAYS_IN_WEEK; offset += 1) {
