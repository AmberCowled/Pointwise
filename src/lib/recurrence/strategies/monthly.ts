...
import { getDateTimeParts, mergeDateAndTime, startOfDay, toDateKey } from "@pointwise/lib/datetime";
import { RECURRENCE_CONFIG } from "../config";
import { isTimeInFuture } from "../filters";
import type { NextOccurrenceInput, RecurrenceConfig } from "../types";
...
export class MonthlyRecurrenceStrategy implements RecurrenceStrategy {
	generateOccurrences(
		...
		const monthDays =
			config.monthDays && config.monthDays.length > 0 ? config.monthDays : [baseParts.day];
		...
		while (occurrenceCount < maxOccurrences) {
			...
			for (const day of monthDays) {
				...
				const candidateParts = getDateTimeParts(new Date(Date.UTC(year, month - 1, day)), timeZone);
				const candidate = new Date(
					Date.UTC(candidateParts.year, candidateParts.month - 1, candidateParts.day),
				);
	...
	findNextOccurrence(input: NextOccurrenceInput): Date | null {
		...
		const baseParts = getDateTimeParts(base, timeZone);
		const targetMonthDays = monthDays && monthDays.length > 0 ? monthDays : [baseParts.day];

		let current = new Date(base);
		const todayKey = toDateKey(new Date(), timeZone);
		const maxMonths = RECURRENCE_CONFIG.monthly.maxMonthsToSearch;

		for (let monthOffset = lastTaskDate ? 1 : 0; monthOffset < maxMonths; monthOffset += 1) {
			const currentParts = getDateTimeParts(current, timeZone);
			const year = currentParts.year;
			const month = currentParts.month;
