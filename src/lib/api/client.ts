...
import { ApiError, NetworkError, ServerError, UnauthorizedError, ValidationError } from "./errors";
		...
		try {
			...
			if (!response.ok) {
				...
				if (response.status === 401) {
					...
					const error = new ValidationError(errorMessage, response.status, data);
					return this.handleError(error, onError);
				}
			}
			...
			// Unknown error
			const unknownError = new ApiError("An unexpected error occurred", 0, error);
