...
export default function DeleteTaskModal({ task }: { task: TaskV2 }) {
  const [deleteTask, { isLoading: isDeleting }] = useDeleteTaskMutation();

  const handleDeleteTask = async () => {
    try {
      await deleteTask({ taskId: task.id }).unwrap();
      ModalV2.Manager.close(`delete-task-modal-${task.id}`);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  return (
    <ModalV2 id={`delete-task-modal-${task.id}`} size="lg" loading={isDeleting}>
      <ModalV2.Header title="Delete Task" />
      <ModalV2.Body>
