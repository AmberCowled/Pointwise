					...
					{hasStart ? (
						<div className="space-y-2">
							<Input
								...
								onChange={(event) => onStartDateUpdate(event.target.value || undefined)}
								fullWidth
							/>
							<div>
								<label htmlFor="startTime" className="mb-1 block text-sm font-medium text-zinc-300">
									Start time (optional)
								</label>
								<div
								...
								>
									...
									/>
									<span className="text-sm">{form.startTime || "Select time"}</span>
								</div>
							</div>
						</div>
					) : null}
					...
					{hasDue ? (
						<div className="space-y-2">
							<Input
								...
								onChange={(event) => onDueDateUpdate(event.target.value || undefined)}
								fullWidth
							/>
							<div>
								<label htmlFor="dueTime" className="mb-1 block text-sm font-medium text-zinc-300">
									Due time (optional)
								</label>
								<div
									className="inline-flex items-center rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-zinc-200 transition hover:border-indigo-400/60 hover:bg-indigo-500/10 cursor-pointer"
									onClick={(e) => {
										e.preventDefault();
										if (dueTimeInputRef.current && "showPicker" in dueTimeInputRef.current) {
