	...
	return (
		<>
			...
			{form.recurrence === "weekly" ? (
				<div>
					<label className="mb-2 block text-xs font-medium text-zinc-300">Repeat on</label>
					{errors.recurrenceDays ? (
						<p className="mb-2 text-xs text-rose-400">{errors.recurrenceDays}</p>
					) : null}
			...
			{form.recurrence && form.recurrence !== "none" ? (
				<div className="space-y-3">
					<div className="flex items-center justify-between">
						<label className="text-xs font-medium text-zinc-300">Times of day</label>
						<button
							type="button"
						...
						{(form.timesOfDay ?? []).map((time, index) => (
							<div key={index} className="flex items-center gap-2">
								<Input
									...
									name={`time-${index}`}
									value={time}
									onChange={(event) => onUpdateTimeOfDay(index, event.target.value)}
									size="sm"
									variant="secondary"
