...
import { CUSTOM_CATEGORY_LABEL, MAX_CUSTOM_CATEGORY_LENGTH } from "@pointwise/lib/categories";
...
import type { RecurringTaskData, TaskFormValues } from "./form/types";
...
export default function TaskCreateModal({
	...
	const handleSubmit = async (event: React.FormEvent) => {
		...
		const isConvertingToOneTime =
			isEditingSeries && recurringTaskData && form.recurrence === "none";
		...
		try {
			...
			const message =
				error instanceof Error ? error.message : `Failed to ${action} task. Please try again.`;
              ...
              */}
								...
								(mode === "edit" && editScope === "single" && !task?.sourceRecurringTaskId) ? (
									<RecurrenceFields
										...
										onRecurrenceMonthDaysChange={(days) =>
											handleChange("recurrenceMonthDays", days)
										}
							...
							<aside className="space-y-6">
										...
										task={
											{
												...
												context: form.context || "Add context to describe what success looks like.",
												category:
													selectedCategory === CUSTOM_CATEGORY_OPTION_VALUE
								...
								<div className="space-y-3 rounded-3xl border border-white/5 bg-zinc-900/60 p-5">
									...
									<ul className="space-y-2 text-xs text-zinc-400">
										<li>Reward deep work with higher XP to boost streak momentum.</li>
										<li>Recurring tasks keep your routine sharp—set daily or weekly anchors.</li>
										<li>Break big goals into smaller tasks to earn consistent progress.</li>
									</ul>
								</div>
							</aside>
						</section>
