...
export default function TaskSectionCard({
	...
	const [isCollapsed, setIsCollapsed] = useState(() => {
		if (collapsible && storageKey) {
			const stored = localStorage.getItem(`task-section-collapsed-${storageKey}`);
			if (stored !== null) {
				return stored === "true";
			}
		}
	...
	const handleToggle = () => {
		...
		if (storageKey) {
			localStorage.setItem(`task-section-collapsed-${storageKey}`, String(newState));
		}
	};
	...
	const actionButtons = [
		collapsible && (
			...
			>
				{isCollapsed ? <IoChevronDown className="h-4 w-4" /> : <IoChevronUp className="h-4 w-4" />}
			</button>
		),
		action,
	].filter(Boolean);
	...
	return (
		<Card
			...
			contentClassName={clsx(
				...
				isCollapsed && collapsible ? "max-h-0 overflow-hidden opacity-0" : "opacity-100",
				contentClassName,
			)}
		>
			{children}
