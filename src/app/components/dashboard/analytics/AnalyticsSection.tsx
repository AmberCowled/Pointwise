...
import { AnalyticsCard, AnalyticsRangeSelect, AnalyticsTabs } from "./AnalyticsCard";
...
export default function AnalyticsSection({
	...
	const [snapshot, setSnapshot] = useState<AnalyticsSnapshot>(initialSnapshot ?? liveSnapshot);
	...
	const tabOptions = (Object.keys(ANALYTICS_TAB_LABELS) as AnalyticsTab[]).map((value) => ({
		value,
		label: ANALYTICS_TAB_LABELS[value],
	}));

	const rangeOptions = (Object.keys(ANALYTICS_RANGE_LABELS) as AnalyticsRange[]).map((value) => ({
		value,
	...
	return (
		<AnalyticsCard
			...
			subtitle="Analytics"
			controls={<AnalyticsRangeSelect value={range} options={rangeOptions} onChange={setRange} />}
			...
			{tab === "xp" ? (
				<div className="space-y-6">
					...
					<LineChart
						...
						formatValue={(point) => `+${numberFormatter.format(Math.round(point.value))} XP`}
						locale={locale}
					/>
					<div className="grid grid-cols-3 text-xs text-zinc-500">
						<span>{xpSeries[0]?.label ?? ""}</span>
						<span className="text-center">
							{xpSeries[Math.max(0, Math.floor((xpSeries.length - 1) / 2))]?.label ?? ""}
						</span>
						<span className="text-right">{xpSeries[xpSeries.length - 1]?.label ?? ""}</span>
					</div>
				</div>
			) : tab === "focus" ? (
				<div className="space-y-6">
					...
					<LineChart
						...
						formatValue={(point) => `${numberFormatter.format(Math.round(point.value))} XP/hr`}
						locale={locale}
					/>
					<div className="grid grid-cols-5 text-xs text-zinc-500">
						{[0, 6, 12, 18, 23].map((hour) => (
							<span
								key={hour}
								className={hour === 0 ? "" : hour === 23 ? "text-right" : "text-center"}
							>
				...
				<div className="grid gap-8 lg:grid-cols-[240px,1fr]">
					<div className="relative mx-auto h-48 w-48">
						<div
							...
							style={{
								background: categoryGradient ?? "conic-gradient(#27272a 0deg 360deg)",
							}}
						/>
								...
								>
									<div className="flex items-center justify-between gap-4">
										<div className="flex items-center gap-3">
											...
											/>
											<span className="font-medium text-zinc-100">{slice.category}</span>
										</div>
											...
											{customCategoryBreakdown.map((custom) => (
												...
												>
													<div className="flex items-center gap-3">
														...
														/>
														<span className="font-medium text-zinc-300">{custom.category}</span>
													</div>
