...
export default function SocialAuthButtons({ onLoadingChange }: Props) {
	...
	const handle = useCallback((provider: "google" | "github") => {
		...
		if (!baseUrl) return;
		setPending(provider);
		signIn(provider, { callbackUrl: `${baseUrl}/dashboard` }).finally(() => setPending(null));
	}, []);
	...
	return (
		<div className="grid grid-cols-2 gap-3">
			...
			>
				...
				<span className="text-xs sm:text-sm whitespace-nowrap">Continue with Google</span>
			</Button>
			...
			>
				...
				<span className="text-xs sm:text-sm whitespace-nowrap">Continue with GitHub</span>
			</Button>
		</div>
	);
}
