...
export interface ModalProps
	 ...
	 * Used to open/close the modal via Modal.Manager
	 */
	id: string;
}

/**
 * Modal - High-level modal component with declarative API
 *
 ...
 *
 * **Static Properties:**
 * - `Modal.Provider` - Provider component for app root
 * - `Modal.Manager` - Singleton manager for opening/closing modals
 * - `Modal.Confirm` - Promise-based confirm modal
 * - `Modal.Alert` - Promise-based alert modal
 * - `Modal.Prompt` - Promise-based prompt modal
 *
 * @example
 * ```tsx
 ...
 * <Modal id="createProject" size="md" closeOnOverlayClick={false}>
 *   <Modal.Header title="Create Project" showCloseButton />
 *   <Modal.Body>
 *     <form>Form content</form>
 *   </Modal.Body>
 *   <Modal.Footer>
 *     <Button onClick={() => Modal.Manager.close("createProject")}>Cancel</Button>
 *     <Button onClick={handleSubmit}>Create</Button>
 *   </Modal.Footer>
 * </Modal>
 *
 * // Open modal from anywhere
 * <Button onClick={() => Modal.Manager.open("createProject")}>
 *   Create Project
 * </Button>
 * ```
 */
...
interface ModalComponent {
	/**
	 * Modal component - High-level modal with declarative API
	 *
	 * @param props - Modal props including id, size, and all ModalContainer props
 ...
 *
 * **Props:**
 * - `id: string` - Required unique ID for manager operations (used to open/close via Modal.Manager)
 * - `size?: "sm" | "md" | "lg" | "xl" | "2xl" | "6xl" | "fullscreen"` - Modal size (default: "md")
 * - `initialFocusRef?: React.RefObject<HTMLElement>` - Element to focus when modal opens
 * - `className?: string` - Custom className for the dialog container
 * - `panelClassName?: string` - Custom className for the dialog panel
 ...
 * ```
 */
const Modal = (({ id, children, ...props }: ModalProps) => {
	return (
