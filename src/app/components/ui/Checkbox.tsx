...
const variantStyles: Record<CheckboxVariants, string> = {
	primary: "border-white/10 bg-white/5 checked:bg-indigo-500 checked:border-indigo-500",
	secondary: "border-white/10 bg-zinc-900 checked:bg-fuchsia-500/20 checked:border-fuchsia-500/50",
	danger: "border-rose-400/60 bg-rose-500/10 checked:bg-rose-500 checked:border-rose-500",
};
...
const variantErrorStyles: Record<CheckboxVariants, string> = {
	...
	secondary: "border-rose-400/60 focus:border-rose-400/80 focus:ring-rose-500/40",
	...
	return (
		<div className="space-y-2">
			<div className="inline-block">
				{hasHeader && <InputHeader htmlFor={checkboxId} required={required} />}

				<div className={clsx("relative flex items-center gap-3", hasHeader && "mt-1")}>
			...
			{errorMessage && <p className="text-xs font-medium text-rose-400">{errorMessage}</p>}
		</div>
	);
});
