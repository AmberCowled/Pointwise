 ...
 * MenuItem Component
 *
 * A menu item that can render as either a link or button, with support for:
 * - Icons, labels, descriptions, and keyboard shortcuts
 * - Badges and badge counts
 * - Loading, disabled, selected, and danger states
 * - Nested submenus (pass a Menu component as children)
...
export interface MenuItemProps {
	 ...
	 * Note: If both `href` and `onClick` are provided, `href` takes precedence
	 */
	onClick?: () => void;
	/**
	 * For link items: href (Next.js Link)
	 * Note: If both `href` and `onClick` are provided, `href` takes precedence
	 */
	href?: string;
	/**
	...
	const menuItemContent = hasCustomContent ? (
		...
		<>
			{loading ? (
				...
				icon && <span className={clsx("shrink-0", iconSizeStyles[size])}>{icon}</span>
			)}
			...
			<div className="shrink-0 ml-auto flex items-center gap-2">
				{shortcut && <span className="text-xs text-zinc-500 font-mono">{shortcut}</span>}
				{badgeContent && <span>{badgeContent}</span>}
				{trailingIcon && (
					<span className={clsx("shrink-0", iconSizeStyles[size])}>{trailingIcon}</span>
				)}
				{hasNestedMenu && (
					<span className={clsx("shrink-0", iconSizeStyles[size])}>
						{submenuPlacement === "right" ? (
