 ...
 * - `md`: Medium (text-sm, px-3 py-2) - default
 * - `lg`: Large (text-base, px-4 py-2.5)
 */
 ...
 * Menu Context Value
 *
 * Context value passed to all menu items to maintain consistent styling
 * and behavior across the menu tree (including nested submenus).
 */
 ...
 * Menu Context
 *
 * React context for sharing menu configuration (variant, size, etc.)
 * with all menu items and submenus.
 */
 ...
 *
 * @returns MenuContextValue with current menu configuration
 * @throws Error if used outside of a Menu component
 *
 * @example
 * ```tsx
 * const { variant, size } = useMenuContext();
 * ```
 */
...
export const selectedItemStyle = "bg-indigo-500/20 text-white border-l-2 border-indigo-400/80";
...
export const dangerItemStyle = "text-rose-200 hover:bg-rose-500/20 hover:text-white";
