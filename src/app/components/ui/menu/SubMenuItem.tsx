 ...
 *
 * Internal component for rendering menu items with nested submenus.
 * Handles hover-based submenu interaction using Popover from Headless UI.
 *
 * @internal This component is used internally by MenuItem when a nested Menu is detected.
 */
...
export function SubMenuItem({
	...
	// Clone nested menu with submenu props (memoized to avoid unnecessary re-renders)
	const submenuElement = useMemo(() => cloneMenuForSubmenu(nestedMenu), [nestedMenu]);
		...
		<HeadlessMenuItem disabled={isDisabled}>
			{() => (
				<Popover>
					...
					>
						{isLinkProps(linkProps) ? (
							<PopoverButton
								{...linkProps}
								disabled={isDisabled}
								className={({ focus }: { focus: boolean }) => getButtonClassName(focus)}
							>
								{menuItemContent}
							</PopoverButton>
						...
						{isSubmenuOpen && (
							<PopoverPanel
								anchor={submenuPlacement === "right" ? "right start" : "left start"}
								static
								className={clsx(
								...
								style={{
									[submenuPlacement === "right" ? "marginLeft" : "marginRight"]: "0.25rem",
									overflowY: "auto",
									overflowX: "visible",
									zIndex: 60,
								}}
