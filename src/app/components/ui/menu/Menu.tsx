...
import React, { Fragment, type ReactNode, useEffect, useLayoutEffect, useRef } from "react";
...
export function Menu({
	...
	useLayoutEffect(() => {
		if (openStateRef.current !== undefined && openStateRef.current !== prevOpenRef.current) {
					...
					return (
						<div className="relative">
							{trigger ? (
								...
								>
									{triggerLabel}
									{!triggerLabel && triggerBadgeCount === undefined && !triggerBadge && (
										<span className="sr-only" aria-label="Open menu">
											Menu
										</span>
									)}
									{(triggerBadge || (triggerBadgeCount !== undefined && triggerBadgeCount > 0)) && (
