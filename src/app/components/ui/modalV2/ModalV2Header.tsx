...
/**
 * Props for ModalV2Header component
 */
export interface ModalV2HeaderProps
...
const titleSizeStyles: Record<NonNullable<ModalV2HeaderProps["size"]>, string> = {
	sm: "text-base",
	md: "text-lg",
	lg: "text-xl",
};

const subtitleSizeStyles: Record<NonNullable<ModalV2HeaderProps["size"]>, string> = {
	sm: "text-xs",
	md: "text-sm",
	lg: "text-base",
};
...
/**
 * ModalV2Header component for modal titles and actions
 *
 * Automatically uses the close function from ModalV2 context when showCloseButton is true.
...
export function ModalV2Header({
	...
	// Require ModalContext - ModalV2Header must be used inside a ModalV2
	...
	return (
		...
		>
			<div className="space-y-1 flex-1">
				...
				{subtitle ? (
					<p className={clsx("text-zinc-500", subtitleSizeStyles[size])}>{subtitle}</p>
				) : null}
				{children}
			</div>
			<div className="flex items-center gap-2">
				{actions}
				{showCloseButton && handleClose && <ModalCloseButton onClose={handleClose} />}
			</div>
		</header>
	);
}
