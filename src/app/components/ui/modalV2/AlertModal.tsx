 ...
 */
export function AlertModalComponent({ id, options }: { id: string; options: AlertModalOptions }) {
	const handleClose = () => {
	...
	return (
		<ModalV2 id={id} size={options.size || "md"}>
			...
			<ModalV2.Footer align="end">
				<Button variant={options.buttonVariant || "primary"} onClick={handleClose}>
...
export async function AlertModal(options: AlertModalOptions): Promise<void> {
	return new Promise((resolve, reject) => {
		...
		ModalRenderQueue.add({
			id,
			component: AlertModalComponent as ComponentType<{ id: string; [key: string]: unknown }>,
			props: { options },
		});
