...
export function Divider({
				...
				{hasLabel ? (
					<>
						<div className="absolute inset-0 flex items-center">
							<span className={clsx("w-full border-t", variantBorderStyles[variant])} />
						</div>
						<div className="relative flex justify-center">
							<span className={clsx("px-2 text-xs font-medium", variantLabelStyles[variant])}>
								{label}
							</span>
						</div>
					</>
				) : (
					<span className={clsx("block w-full border-t", variantBorderStyles[variant])} />
				)}
			</div>
		);
	}
	...
	return (
		<div
			className={clsx("relative flex items-center", spacingStyles[spacing].vertical, className)}
			role="separator"
		...
		>
			{hasLabel ? (
				<div className="flex flex-col items-center h-full">
					<span className={clsx("flex-1 border-l", variantBorderStyles[variant])} />
					<span
						className={clsx(
				...
				<span className={clsx("h-full border-l", variantBorderStyles[variant])} />
			)}
		</div>
	);
}
