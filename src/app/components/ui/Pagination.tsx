...
const baseContainerStyle = "flex flex-col gap-3 sm:gap-4 sm:flex-row sm:items-center";
...
const variantActivePageStyles: Record<PaginationVariant, string> = {
	primary: "border-indigo-400/60 bg-indigo-500/20 text-white shadow-lg shadow-indigo-500/20",
	secondary: "border-fuchsia-500/60 bg-fuchsia-500/20 text-white shadow-lg shadow-fuchsia-500/20",
};
...
function generateDesktopPageNumbers(
	...
	if (currentPage <= halfVisible + 1) {
		...
		const startPage = Math.max(2, currentPage - Math.floor((pagesAroundCurrent - 1) / 2));
		const endPage = Math.min(totalPages - 1, startPage + pagesAroundCurrent - 1);
...
export function Pagination({
	...
	const validatedTotalPages = Math.max(1, Math.ceil(validatedTotalItems / validatedPageSize));
	const validatedCurrentPage = Math.max(1, Math.min(currentPage, validatedTotalPages));
	...
	const startItem =
		validatedTotalItems === 0 ? 0 : (validatedCurrentPage - 1) * validatedPageSize + 1;
	const endItem = Math.min(validatedCurrentPage * validatedPageSize, validatedTotalItems);
	...
	const getMaxPages = (): number => {
		...
		const maxPages = Math.floor(availableForPages / (avgButtonWidth + gapWidth));
	...
	const pageNumbers = isMobile
		? generateMobilePageNumbers(validatedCurrentPage, validatedTotalPages, getMaxPages())
		: generateDesktopPageNumbers(validatedCurrentPage, validatedTotalPages, getMaxPages());
	...
	const useThreeSectionLayout = showItemCount || (showPageSizeSelector && onPageSizeChange);
		...
		>
			...
			<div aria-live="polite" aria-atomic="true" className="sr-only">
				{validatedTotalItems > 0 && (
					<span>
						Page {validatedCurrentPage} of {validatedTotalPages}, showing {startItem} to {endItem}{" "}
						of {validatedTotalItems} items
					</span>
				)}
			</div>
				...
				<div className="hidden sm:flex items-center shrink-0">
					{showItemCount ? (
						...
						<div className="invisible" aria-hidden="true">
							<div className={clsx(sizeTextStyles[size], "text-zinc-500")}>
								<span className="text-zinc-400">0–0</span> <span className="text-zinc-500">of</span>{" "}
										...
										return (
											<button
												...
												className={clsx(
													...
													enableTransitions && "transition-all duration-150 ease-in-out",
			...
			{useThreeSectionLayout && (
				<div className="hidden sm:flex items-center gap-2 shrink-0">
					{showPageSizeSelector && onPageSizeChange ? (
						<>
							<span className={clsx(sizeTextStyles[size], "text-zinc-400")}>Show:</span>
							<InputSelect
