		...
		>
			{hasHeader && (
				<header className="flex flex-wrap items-center justify-between gap-3">
					<div>
						{label && <p className="text-xs uppercase tracking-[0.3em] text-zinc-500">{label}</p>}
						{title && (
							<h2 className={clsx("text-xl font-semibold text-zinc-100", label && "mt-2")}>
								{title}
							</h2>
						)}
					</div>
			...
			<div className={clsx(hasHeader && "mt-5", contentClassName)}>
				{loading ? (
					<div className="flex flex-col items-center justify-center py-12">
						...
						{loadingMessage && <p className="text-sm text-zinc-400 mt-4">{loadingMessage}</p>}
					</div>
				) : (
					children
				)}
