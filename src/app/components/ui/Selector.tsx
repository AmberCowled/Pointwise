...
const sizeStyles: Record<SelectorSize, { padding: string; textSize: string; iconSize: string }> = {
...
export function SelectorOption({
	...
	return (
		<button
			type="button"
			className={clsx("flex flex-col items-center justify-center text-center gap-1", className)}
			{...props}
		>
			{Icon && <Icon className={clsx(sizeStyle.iconSize, "shrink-0")} aria-hidden="true" />}
			<span className={clsx("font-medium", sizeStyle.textSize)}>{label}</span>
			{description && (
				<span className={clsx("mt-1 text-zinc-500", size === "xs" ? "text-[10px]" : "text-xs")}>
					{description}
				</span>
			)}
		</button>
...
const unselectedStyles = "bg-zinc-800/50 border-white/10 text-zinc-400 hover:border-white/20";
...
const Selector = (({
	...
	const options = Children.map(children, (child) => {
		if (!isValidElement<SelectorOptionProps>(child) || child.type !== SelectorOption) {
			return child;
		}
