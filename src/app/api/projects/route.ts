import { createProject, getProjects, serializeProject } from "@pointwise/lib/api/projectsV2";
...
export async function GET(req: Request) {
	return handleProtectedRoute(req, async ({ user }) => {
		...
		const projects = prismaProjects.map((project) => serializeProject(project, user.id));
