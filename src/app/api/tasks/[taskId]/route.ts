...
import { userCanEditTasks, verifyProjectAccess } from "@pointwise/lib/api/project-access";
...
import { isTaskInstance, isTaskTemplate } from "@pointwise/lib/tasks/utils";
	...
	return handleRoute(req, async () => {
		...
		const result = await prisma.$transaction(async (tx) => {
			...
			const isChangingRecurrence =
				scope === "series" && hasRecurrenceFields && isTemplate && newRecurrence !== "none";
