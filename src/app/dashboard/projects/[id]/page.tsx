...
export default async function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
	...
	const headerLocale = headerStore.get("accept-language")?.split(",")[0]?.trim() ?? undefined;
	...
	const tasks: DashboardTask[] = tasksFromDb.map((task) => {
		...
		let recurrencePattern: DashboardTask["recurrencePattern"];
		...
		return {
			...
			xp: task.xpValue ?? 0,
			status:
				(task.status as DashboardTask["status"]) || (task.completedAt ? "completed" : "scheduled"),
