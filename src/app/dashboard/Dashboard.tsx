...
export default function Dashboard({
	...
	const { handleSubmitTask, handleDeleteTask, handleCompleteWithLoading, completingId } =
		useTaskOperations({
	...
	const { scheduledTasks, optionalTasks, overdueTasks, selectedDateLabel, selectedDateInputValue } =
		useTaskFilters(
			tasksWithInstances,
			selectedDate,
			formatLocale,
			...
			<div className="relative z-10 mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-8 px-4 py-10 sm:px-6 lg:px-8">
				<header className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
					<div className="space-y-1">
						<p className="text-xs uppercase tracking-[0.3em] text-zinc-500">{displayToday}</p>
